...
@brief Реализация класса окна настройки фильтров
...
: QDialog(parent), ui(new Ui::FiltersDialog)
{
    ...
    ui->cb_UseFilters->setChecked(Utility::GetSettingInt(settings, "UseFilters", 1) != 0);
    int filtersCount = Utility::GetSettingInt(settings, "FiltersCount", 1);

    for (int i = 0; i < filtersCount; i++)
    {
        OnAddedFilter(settings.value(QString("Filter%1_Type").arg(i), "Butterworth").toString(),
                      Utility::GetSettingInt(settings, QString("Filter%1_TypeId").arg(i), 1),
                      Utility::GetSettingInt(settings, QString("Filter%1_Order").arg(i), 2),
                      Utility::GetSettingInt(settings, QString("Filter%1_RangeMin").arg(i), 1),
                      Utility::GetSettingInt(settings, QString("Filter%1_RangeMax").arg(i), 45));
    }
}
//----------------------------------------------------------------------------------
